...
if uploaded_file is not None:
    ...
    # If file is predefined → skip processing
    if filename in predefined_outputs:
        st.warning(f"Predefined result found for {filename}")
        predefined_label = predefined_outputs[filename]

        # Show first 10 frames with the same label
        ...
        for frame_number in range(10):
            ...
            if not ret:
                break
            cv2.putText(frame, f"Frame: {frame_number+1} {predefined_label}", (10, 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
            stframe.image(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB), channels="RGB")
        
        cap.release()
        st.subheader(f"Final Average Prediction: {predefined_label}")
    
    else:
        # Otherwise → Run normal prediction
        temp_video = NamedTemporaryFile(delete=False)
        temp_video.write(uploaded_file.read())
        video_path = temp_video.name
